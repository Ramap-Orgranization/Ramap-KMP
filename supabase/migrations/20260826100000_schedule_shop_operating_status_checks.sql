alter table public.shop_operating_status
    add column next_check_at timestamptz not null default now(),
    add column check_interval_minutes integer not null default 60 check (check_interval_minutes > 0),
    add column check_reason text not null default 'initial',
    add column consecutive_failures integer not null default 0 check (consecutive_failures >= 0);

create index shop_operating_status_next_check_at_idx
on public.shop_operating_status (next_check_at);

drop function if exists public.claim_due_shop_operating_statuses(integer);

create function public.claim_due_shop_operating_statuses(batch_limit integer default 10)
returns table (
    id text,
    name text,
...
begin
    ...
    select id::text, naver_place_url, now(), 60, 'initial', 0
    from public.shops where is_visible and naver_place_url is not null
    ...
    select shops.id::text, shops.name, shops.naver_place_url, shops.business_hours_weekly, shops.business_hours_break_times, status.check_interval_minutes, status.check_reason, claimed.consecutive_failures
