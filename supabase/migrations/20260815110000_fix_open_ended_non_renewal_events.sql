...
as $function$
    select case
        when event_type = 'store_renewal' then start_date >= today
...
as $function$
    select today >= start_date
        and case
            when event_type = 'store_renewal' then today <= start_date
...
immutable
as $function$
    select start_date < period_end
        and case
            when event_type = 'store_renewal' then start_date >= period_start
            else end_date is null or end_date >= period_start
        end;
$function$;
...
current_events as (
    ...
    cross join bounds
    where public.event_overlaps_period(
        event.event_type,
        event.start_date,
        event.end_date,
        bounds.month_start,
        bounds.next_month_start
    )
),
reminder_dates as (
